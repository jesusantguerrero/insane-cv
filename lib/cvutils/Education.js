...
class Education {
   ...
   return prompt(this.menu).then((a) => {
      switch (a.order) {
        ...
        case 'Edit':{
          ...
          return prompt(list)
            .then((r) => {
             const index = items.findIndex((education) => {
               return education.degree === r.index
              })
              return this.edit(curriculum, r.index, items[index])
            })
            .then((res) => true)
          }
          break
        case 'Delete':  {
          ...
          return prompt(list)
            .then((r) => this.delete(curriculum, r.index))
            .then((res) => true)
          }
          break
        default:
  ...
  loadEducation(curriculum, mode){
    ...
    const listMode = items.map((item, i) =>  item.degree)
    const list = [{
      type: 'list',
      name: 'index',
      choices: listMode,
